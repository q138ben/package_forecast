...
def plot_test_period_zoom(location: str, test_df: pd.DataFrame,
    ...
    if len(future_forecast) > 0:
        ax.plot(future_forecast['ds'], future_forecast['yhat'], 
                color=future_forecast_color, linewidth=2, marker='s', markersize=4,
                label='Future Forecast')
        ax.fill_between(future_forecast['ds'], 
                       future_forecast['yhat_lower'], 
                       future_forecast['yhat_upper'],
                       color=future_ci_color, alpha=0.2, label='95% CI')
    ...
    ax.set_title(f'Location {location}: Test Period + Future Forecast', 
    ...
    total_days = (future_forecast['ds'].max() - test_df['ds'].min()).days if len(future_forecast) > 0 else len(test_df)
    interval = max(5, total_days // 10)

    ax.text(0.02, 0.98, metrics_text, transform=ax.transAxes, fontsize=11,
            verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    ax.set_title(f'Location {location}: Test Period - Actual vs Forecast', 
                 fontsize=14, fontweight='bold')
    ax.set_xlabel('Date')
    ax.set_ylabel('Package Volume')
    ax.legend(loc='upper right')
