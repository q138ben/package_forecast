...
def train_location_model(
    ...
    cv_results = run_time_series_cv(
        location, train_df, n_folds=n_cv_folds, test_size=test_size
    )
    ...
    with open(results_file, "w") as f:
        ...
        results_copy["metadata"]["start_date"] = results_copy["metadata"][
            "start_date"
        ].strftime("%Y-%m-%d")
        results_copy["metadata"]["end_date"] = results_copy["metadata"][
            "end_date"
        ].strftime("%Y-%m-%d")
