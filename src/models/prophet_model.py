...
def create_prophet_model(location: str, n_days: int, verbose: bool = True) -> Prophet:
    ...
    model.add_regressor(
        name="is_weekend", prior_scale=100.0, mode="multiplicative", standardize=False
    )

    # Separate Saturday/Sunday effects (useful when only Saturdays are near-zero).
    model.add_regressor(
        name="is_saturday", prior_scale=100.0, mode="multiplicative", standardize=False
    )
    model.add_regressor(
        name="is_sunday", prior_scale=100.0, mode="multiplicative", standardize=False
    )
    ...
    if verbose:
        if yearly_seasonality:
            ...
            print(
                f"Location {location}: Disabled yearly seasonality (only {n_days} days)"
            )
        print(f"Location {location}: Added Swedish holidays")
        print(
            f"Location {location}: Added weekend regressors (multiplicative) + flexible weekly seasonality"
        )
