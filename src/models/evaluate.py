...
def calculate_metrics(
    ...
    if lower is not None and upper is not None:
        ...
        if np.any(valid_mask):
            coverage = (
                np.mean(
                    (y_true[valid_mask] >= lower[valid_mask])
...
def seasonal_naive_forecast(
    ...
    if history.size == 0:
        raise ValueError(
            "Training data must contain at least one observation for baseline forecast."
        )
...
def evaluate_naive_baseline(
    train_df: pd.DataFrame, test_df: pd.DataFrame, seasonal_period: int = 7
) -> Dict[str, float]:
...
def time_series_cv_split(
    df: pd.DataFrame, n_folds: int = 5, test_size: int = 30, gap: int = 0
) -> List[Dict]:
    """
        ...
        splits.append(
            {
                ...
                "train_dates": (
                    df.iloc[0]["ds"].strftime("%Y-%m-%d"),
                    df.iloc[train_end_idx - 1]["ds"].strftime("%Y-%m-%d"),
                ),
                "val_dates": (
                    df.iloc[val_start_idx]["ds"].strftime("%Y-%m-%d"),
...
def run_time_series_cv(
    ...
    for split in splits:
        ...
        folds.append(
            {**split, "metrics": prophet_metrics, "baseline_metrics": baseline_metrics}
        )
